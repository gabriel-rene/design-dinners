...
test.describe("Admin CRUD smoke", () => {
  test("admin creates content that reaches the landing, and delete keeps speakers", async ({
    ...
    // Restore the seeded baseline so the landing spec's exact-count assertions
    // (and re-runs of this spec) keep seeing only seeded content. The shared
    // Supabase DB is mutable global state; leaving residue would break others.
    await test.step("clean up created speaker and resource", async () => {
      await page.goto("/admin/speakers");
      await page
        .getByRole("listitem")
